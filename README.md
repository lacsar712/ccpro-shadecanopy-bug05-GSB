...
4. **ClimateLog**：zoneId / recordedAt / tempC / humidityPct / parUmol / co2Ppm；**humidityPct ∈ [20, 100]**；同一分区同一采样时刻仅允许一条记录：重复创建/更新到已占用时刻返回 400（并发竞争返回 409）并给出说明，不覆盖旧值；删除旧记录后该时刻可重新使用
